...
* **Ziel:** Ein präziser Labor-Test der eigenen Hardware am eigenen Standort mit dem eigenen Rufzeichen. Dies geht weit über den klassischen "Antenne A vs. Antenne B" Vergleich hinaus: Solange alle anderen Parameter identisch bleiben, können Sie hier jede beliebige Variable isoliert testen. Vergleichen Sie Empfänger gegen Empfänger (RX vs. RX), Transceiver gegen Transceiver (TX vs. TX), unterschiedliche Baluns/Speiseleitungen oder evaluieren Sie die exakte Aufbaustelle einer Antenne innerhalb desselben Grundstücks (Standort A vs. Standort B). WSPRadar spaltet sich hierfür je nach Test-Richtung in zwei spezielle Rechenpfade auf:
* **Der RX A/B-Test (Simultan):** Zwei parallel laufende Empfänger (SDRs) werten gleichzeitig WSPR-Signale aus. **Der Trick:** Damit das WSPR-Netzwerk die synchronen Spots Ihrer Empfänger nicht als Duplikate löscht, müssen Sie in Ihrer Empfangssoftware (z.B. WSPRdaemon) unterschiedliche 6-stellige Maidenhead-Locators angeben (z.B. Target-SDR an `JN37AA` und Ref-SDR an `JN37AB`). In WSPRadar geben Sie im Self-Test Modus diese beiden Grid-Locators an. Das Tool generiert dann via *Spot-by-Spot (Synchronous)* Mathematik exakte Joint-Spots für den ultimativen Empfangsantennen-Vergleich.
  * *Wissenschaftliche Rechtfertigung des Locator-Shifts:* Das Standard-WSPR-Protokoll überträgt über die Luft lediglich einen 4-stelligen Basis-Locator (z. B. `JN37`, ein Raster von $2^\circ \times 1^\circ$, ca. $150 \times 111$ km in mittleren Breiten). Die Erweiterung auf 6 Stellen (ein Sub-Grid von $5' \times 2.5'$, ca. $6 \times 4$ km) wird ausschließlich durch die lokale Empfangssoftware beim Upload über das Internet hinzugefügt. Eine Verschiebung im 6-stelligen Bereich (z.B. von `AA` auf `AB`) verändert das HF-Protokoll nicht und beeinträchtigt die wissenschaftliche Integrität der weltweiten WSPR-Datenbank bezüglich Makro-Ausbreitungsmodellen in keiner Weise. **Achtung:** Der 4-stellige Basis-Locator darf jedoch unter keinen Umständen verfälscht werden.
* **Der TX A/B-Test (Sequenziell / Time-Slicing):** Sie verwenden einen Transceiver, der periodisch zwischen Sende-Antenne A und B umschaltet. Da Antenne A und B *niemals* im selben Slot senden, nutzt WSPRadar die **Asynchronous Math (Time-Averaging)**: Es spaltet Ihre Daten anhand der Startminuten (Gerade: 00, 04, 08 / Ungerade: 02, 06, 10) auf und bildet Langzeit-Mediane, bevor das $\Delta$ SNR berechnet wird. 
...
* **Min. Stationen pro Karten-Segment:** Dient als **Basis-Filter** für alle Karten. Dieser Filter verhindert, dass einzelne Zufallsempfänge das Bild verfälschen, indem Segmente mit zu wenigen Stationen komplett ausgeblendet werden. Zwingend erforderlich zur Abschwächung des "WSPR-Kollisions-Problems" (Überlastung auf Empfängerseite, bei der schwache Signale durch starke Interferenzen verloren gehen).
